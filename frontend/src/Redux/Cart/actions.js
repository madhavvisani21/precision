...
let isadded = (id) => (dispatch) => {
  ...
  axios
    .get(`https://localhost:8000/cart/isadded/${id}`, {
      headers: {
        token,
      },
    })
...
let increaseCartData = (id) => (dispatch) => {
  let token = JSON.parse(localStorage.getItem("rentifyToken"));
  axios
    .get(`https://localhost:8000/cart/cartquantityadd/${id}`, {
      headers: {
        token,
      },
    })
...
let decreaseCartData = (id) => (dispatch) => {
  let token = JSON.parse(localStorage.getItem("rentifyToken"));
  axios
    .get(`https://localhost:8000/cart/cartquantityreduce/${id}`, {
      headers: {
        token,
      },
    })
...
let deleteCartItem = (id) => (dispatch) => {
  let token = JSON.parse(localStorage.getItem("rentifyToken"));
  axios
    .delete(`https://localhost:8000/cart/delete/${id}`, {
      headers: {
        token,
      },
    })
